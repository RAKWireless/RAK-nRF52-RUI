...
/* Serial Port Command */
    ...
    {ATCMD_PWORD,    /*11*/         At_Pword,              0, "set the serial port locking password (max 8 char)"},
...
/* LoRaWAN Keys and IDs */
    {ATCMD_APPEUI,   /*16*/         At_AppEui,             0, "get or set the application EUI (8 bytes in hex)"},
    {ATCMD_APPKEY,   /*17*/         At_AppKey,             0, "get or set the application key (16 bytes in hex)"},
    {ATCMD_APPSKEY,  /*18*/         At_AppSKey,            0, "get or set the application session key (16 bytes in hex)"},
    {ATCMD_DADDR,    /*19*/         At_DevAddr,            0, "get or set the device address (4 bytes in hex)"},
    {ATCMD_DEUI,     /*20*/         At_DevEui,             0, "get or set the device EUI (8 bytes in hex)"},
    {ATCMD_NETID,    /*21*/         At_NetId,              0, "get the network identifier (NetID) (3 bytes in hex)"},
    {ATCMD_NWKSKEY,  /*22*/         At_NwkSKey,            0, "get or set the network session key (16 bytes in hex)"},
/* LoRaWAN Joining and Sending */
    {ATCMD_CFM,      /*23*/         At_CfMode,             0, "get or set the confirm mode (0 = off, 1 = on)"},
    {ATCMD_CFS,      /*24*/         At_CfStatus,           0, "get the confirmation status of the last AT+SEND (0 = failure, 1 = success)"},
    {ATCMD_JOIN,     /*25*/         At_Join,               0, "join network"},
    {ATCMD_NJM,      /*26*/         At_NwkJoinMode,        0, "get or set the network join mode (0 = ABP, 1 = OTAA)"},
    {ATCMD_NJS,      /*27*/         At_NwkJoinStatus,      0, "get the join status (0 = not joined, 1 = joined)"},
    ...
    {ATCMD_LPSEND,   /*83*/         At_Lpsend,             0, "send long packet data (max 1024 bytes)"},
...
/* LoRaWAN Network Management */
    ...
    {ATCMD_CLASS,    /*32*/         At_Class,              0, "get or set the device class (A = class A, B = class B, C = class C)"},
    {ATCMD_DCS,      /*33*/         At_DCS,                0, "get the ETSI duty cycle setting (0 = disabled, 1 = enabled)"},
    {ATCMD_DR,       /*34*/         At_DataRate,           0, "get or set the data rate"},
    ...
    {ATCMD_RX2DR,    /*40*/         At_RxWin2DataRate,     0, "get or set the RX2 window data rate"},
    {ATCMD_RX2FQ,    /*41*/         At_RxWin2Freq,         0, "get the RX2 window frequency (Hz)"},
    {ATCMD_TXP,      /*42*/         At_TxPower,            0, "get or set the transmitting power"},
    {ATCMD_LINKCHECK,/*43*/         At_LinkCheck,          0, "get or set the link check setting (0 = disabled, 1 = once, 2 = everytime)"},
/* LoRaWAN Class B */
    {ATCMD_PGSLOT,   /*44*/         At_PingSlot,           0, "get or set the unicast ping slot periodicity (0-7)"},
    {ATCMD_BFREQ,    /*45*/         At_BeaconFreq,         0, "get the data rate and beacon frequency (MHz)"},
    {ATCMD_BTIME,    /*46*/         At_BeaconTime,         0, "get the beacon time (seconds since GPS Epoch time)"},
    {ATCMD_BGW,      /*47*/         At_BGW,                0, "get the gateway GPS coordinate, NetID and GwID"},
    {ATCMD_LTIME,    /*48*/         At_LocalTime,          0, "get the local time"},
/* LoRaWAN Information */
...
#endif
    {ATCMD_BAND,     /*54*/         At_Band,               0, "get or set the active region (0 = EU433, 1 = CN70, 2 = RU864, 3 = IN865, 4 = EU868, 5 = US915, 6 = AU915, 7 = KR920, 8 = AS923)"},
/* LoRaWAN P2P */
    ...
    {ATCMD_PCR,      /*59*/         At_P2pCR,              0, "configure P2P Code Rate(0=4/5, 1=4/6, 2=4/7, 3=4/8)"},
    {ATCMD_PPL,      /*60*/         At_P2pPL,              0, "configure P2P Preamble Length (5-65535)"},
    {ATCMD_PTP,      /*61*/         At_P2pTP,              0, "configure P2P TX Power(5-22)"},
    {ATCMD_PSEND,    /*62*/         At_P2pSend,            0, "send data in P2P mode"},
    {ATCMD_PRECV,    /*63*/         At_P2pRecv,            0, "enter P2P RX mode for a period of time (ms)"},
    {ATCMD_PCRYPT,   /*64*/         At_P2pCrypt,           0, "get or set the encryption status of P2P mode"},
    {ATCMD_PKEY,     /*65*/         At_P2pKey,             0, "get or set the encryption key of P2P mode (8 bytes in hex)"},
    {ATCMD_P2P,      /*66*/         At_P2p,                0, "get or set all P2P parameters"},
...
/* LoRaWAN RF Test */
    ...
    {ATCMD_TTX,    /*79*/           At_Ttx,                0, "set number of packets to be sent for PER RF TX test"},
    {ATCMD_TRX,    /*80*/           At_Trx,                0, "set number of packets to be received for PER RF RX test"},
    {ATCMD_TCONF,  /*81*/           At_Tconf,              0, "configure LoRa RF test"},
    {ATCMD_TTH,    /*82*/           At_Tth,                0, "start RF TX hopping test from Fstart to Fstop, with Fdelta steps"},
