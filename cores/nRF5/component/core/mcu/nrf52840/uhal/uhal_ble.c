...
{
    uint32_t    dtm_error_code = DTM_ERROR_ILLEGAL_CONFIGURATION;
    uint16_t command = 0;
    char c[2]={'\0'};
    uhal_stop_ble();
    ...
    {
        c[0] = raw_data[i];
        command += strtol(c,NULL,16) <<(12-4*i);
    }
    
    //command = strtol(raw_data,NULL,16);
    if(is_dtm_init == 0)
    {
        is_dtm_init = 1;
        dtm_error_code = dtm_init();
    }
    dtm_error_code = dtm_cmd(command);
...
{
    dtm_event_t result;
    bool event_get = dtm_event_get(&result);
    //recv[0] = sprintf(recv,"%x",(result >> 8) & 0xFF);
    //recv[1] = result & 0xFF;
    /*udrv_serial_log_printf("dtm_event_get:%u\r\n",event_get);
    udrv_serial_log_printf("dtm_event_get:%u\r\n",(result >> 8) & 0xFF);
    udrv_serial_log_printf("dtm_event_get:%u\r\n",result & 0xFF);
    udrv_serial_log_printf("dtm_event_get:%u\r\n",recv[0]);
    udrv_serial_log_printf("dtm_event_get:%u\r\n",recv[1]);
    ...
    {
        *recv_bytes = 2;
        //sprintf(recv,"%08x",result);
        recv[0] = (result >> 8) & 0xFF;
        recv[1] = result & 0xFF;
        return UDRV_RETURN_OK;
    }
    else
