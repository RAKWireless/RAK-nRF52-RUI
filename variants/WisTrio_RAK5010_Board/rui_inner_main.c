...
static uint64_t reinit_start, reinit_end;
bool keep_bg96_on;
static bool cellular_last_status = false;
bool cellular_parser_enable;
...
void rui_event_handler_func(void *data, uint16_t size) {
    ...
    static uint8_t Uart0LastRecvEnter = 0;
    static uint8_t Uart1LastRecvEnter = 0;
    switch (event->request) {
        ...
        case UDRV_SYS_EVT_OP_SERIAL_UART:
        {
            if (service_nvm_get_mode_type_from_nvm(SERIAL_UART1) != SERVICE_MODE_TYPE_CUSTOM) {
                while (udrv_serial_read_available(SERIAL_UART1) > 0 || Uart1LastRecvEnter ) {
                    uint8_t Buf[2];
                    if( Uart1LastRecvEnter == 0){
                        int32_t count = udrv_serial_read (SERIAL_UART1, Buf, 1);
                    }else{
                        Buf[0] = 0x0A;
                        Uart1LastRecvEnter = 0;
                    }
                    ...
                    switch (service_nvm_get_mode_type_from_nvm(SERIAL_UART1)) {
                        case SERVICE_MODE_TYPE_CLI:
                        {
                            if ((state = udrv_serial_get_lock_state(SERIAL_UART1)) == SERIAL_WLOCK_OPEN) {
                                if( ( Buf[0] == 0x0A || Buf[0] == 0x0D) && udrv_serial_read_available(SERIAL_UART1) > 0)
                                {
                                    udrv_serial_read(SERIAL_UART1, Buf+1, 1);
                                    if( Buf[1]== 0x0A || Buf[1] == 0x0D)
                                    {
                                        Uart1LastRecvEnter = 1;
                                    }
                                    else
                                    {
                                        service_mode_cli_handler(SERIAL_UART1, Buf[0]);
                                        service_mode_cli_handler(SERIAL_UART1, Buf[1]);
                                    }
                                }
                                else
                                {
                                ...
                                goto STEP_UART1_CHAR_HANDLE_END;
            ...
            else if (service_nvm_get_mode_type_from_nvm(SERIAL_UART1) == SERVICE_MODE_TYPE_CUSTOM)
            {
                bool uart1_active = false;
#ifdef AUTO_DISPLAY_CELLULAR_RESPONSE
                while (udrv_serial_read_available(SERIAL_UART1) > 0) {
                    char Buf[1];
                    udrv_serial_read(SERIAL_UART1, Buf, 1);
                    udrv_serial_log_printf("%c", Buf[0]);
                    uart1_active = true;
                }
#else
                ...
                if (uart1_active == true && service_nvm_get_auto_sleep_time_from_nvm() && keep_bg96_on == true) {
                    NRF_LOG_INFO("Continue keeping BG96 on");
                    udrv_system_user_app_timer_stop ();
                    udrv_system_user_app_timer_start (RAK5010_BG96_LPM_MIN_ACTIVE_TIME, NULL);
                }
            }
            STEP_UART1_CHAR_HANDLE_END:
            if (service_nvm_get_mode_type_from_nvm(SERIAL_UART0) != SERVICE_MODE_TYPE_CUSTOM) {
                while (udrv_serial_read_available(SERIAL_UART0) > 0 || Uart0LastRecvEnter) {
                    uint8_t Buf[2];
                    if( Uart0LastRecvEnter == 0){
                        int32_t count = udrv_serial_read (SERIAL_UART0, Buf, 1);
                    }else{
                        Buf[0] = 0x0A;
                        Uart0LastRecvEnter = 0;
                    }
                    SERIAL_WLOCK_STATE state;
                    switch (service_nvm_get_mode_type_from_nvm(SERIAL_UART0)) {
                        case SERVICE_MODE_TYPE_CLI:
                        {
                            if ((state = udrv_serial_get_lock_state(SERIAL_UART0)) == SERIAL_WLOCK_OPEN) {
                                if( ( Buf[0] == 0x0A || Buf[0] == 0x0D) && udrv_serial_read_available(SERIAL_UART0) > 0)
                                {
                                    udrv_serial_read(SERIAL_UART0, Buf+1, 1);
                                    if( Buf[1]== 0x0A || Buf[1] == 0x0D)
                                    {
                                        Uart0LastRecvEnter = 1;
                                    }
                                    else
                                    {
                                        service_mode_cli_handler(SERIAL_UART0, Buf[0]);
                                        service_mode_cli_handler(SERIAL_UART0, Buf[1]);
                                    }
                                }
                                else
                                {
                                ...
                                goto STEP_UART0_CHAR_HANDLE_END;
                            } else if (state == SERIAL_WLOCK_LOCKED) {
                ...
                while (udrv_serial_read_available(SERIAL_USB0) > 0) {
                    ...
                    switch (service_nvm_get_mode_type_from_nvm(SERIAL_USB0)) {
                        case SERVICE_MODE_TYPE_CLI:
                        {
                            ...
                            {
                                service_mode_cli_cellular_handler(SERIAL_UART0, Buf[0]);
                            }
                            else
#endif
...
{
    NRF_LOG_INFO("End keeping BG96 on");
    keep_bg96_on = false;
...
void rui_init(void)
{
    ...
    udrv_system_user_app_timer_create((timer_handler)rui_user_application_timer_handler, HTMR_ONESHOT);
    Gsm_Init();
...
{
    if (reinit_start == 0) {
        reinit_start = udrv_rtc_get_timestamp((RtcID_E)SYS_RTC_COUNTER_PORT);
        reinit_end = reinit_start + 1000;
    }
    if (udrv_rtc_get_timestamp((RtcID_E)SYS_RTC_COUNTER_PORT) > reinit_end) {
        if(cellular_last_status == false && get_bg96_status() == true) {
            udrv_serial_init(SERIAL_UART1, 115200, SERIAL_WORD_LEN_8, SERIAL_STOP_BIT_1, SERIAL_PARITY_DISABLE, SERIAL_TWO_WIRE_NORMAL_MODE);
        }
...
void rui_system_thread(void)
{
    while (1) {
        ...
        if (service_nvm_get_auto_sleep_time_from_nvm() && uhal_sched_run_queue_empty()) {
            if (keep_bg96_on == false) {
                if (udrv_sleep_ms(0) == UDRV_RETURN_OK) {
                    NRF_LOG_INFO("Start keeping BG96 on");
                    keep_bg96_on = true;
                    udrv_system_user_app_timer_start(RAK5010_BG96_LPM_MIN_ACTIVE_TIME, NULL);
                }
            } else {
