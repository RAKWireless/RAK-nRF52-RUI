 ...
 *  txpower, broadcastName, advertiseInterval, advertise, advertisement status, mac.
 *
***/
...
void setup()
{
    ...
    if (get_dav_status) {
        ...
        if (!(ret = api.ble.settings.advertiseInterval.set(2000))) {
            Serial.printf("BLE Configuration - set advertise interval is incorrect! \r\n");
            return;
        }
        #ifdef rak11720
        char dev_name[13] =
        ...
        #endif
        if (!(ret = api.ble.settings.broadcastName.set(dev_name, strlen(dev_name)))) {
