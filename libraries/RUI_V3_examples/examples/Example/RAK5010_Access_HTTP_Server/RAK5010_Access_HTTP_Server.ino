...
//    }
    ...
    // Configure Parameters for HTTP(S) Server - contextid
    command = "AT+QHTTPCFG=\"contextid\",1\r";
    bg96_write(command.c_str());

    // Configure Parameters for HTTP(S) Server - responseheader
    command = "AT+QHTTPCFG=\"responseheader\",1\r";
    bg96_write(command.c_str());

    // Set URL of HTTP(S) Server
    command = "AT+QHTTPURL=24,80\r";
    bg96_write(command.c_str());

    command = "https://www.google.com/\r";
    bg96_write(command.c_str());
}
